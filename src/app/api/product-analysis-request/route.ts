...
export async function POST(request: NextRequest) {
  ...
  if (body.inputType === "url") {
    try {
      ...
      inputValue = await resolvePublicUrl(inputValue);
  ...
  try {
    ...
    const insert = await db("product_analysis_requests", {
      method: "POST", headers: { Prefer: "return=representation" },
      body: JSON.stringify({ session_id: sessionId, input_type: body.inputType, input_value: storedInput, product_id: product.id, status }),
    }, key);
    ...
    return NextResponse.json({
      ok: true, requestId, sessionId, productId: product.id, productName: product.canonical_name ?? storedInput,
      status, resolvedByBrand, resolvedByAlias, resolvedUrl: body.inputType === "url" && inputValue !== body.inputValue?.trim() ? inputValue : undefined,
      message: status === "completed" ? "이미 분석된 상품 결과가 있습니다."
        : resolvedByBrand || resolvedByAlias ? `${body.inputValue}을(를) ${product.canonical_name} 상품으로 연결해 분석 대기에 등록했습니다.`
        : "상품 분석 요청을 등록했습니다.",
    });
  } catch (error) {

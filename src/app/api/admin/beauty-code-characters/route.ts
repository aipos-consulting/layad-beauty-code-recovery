...
async function db(path: string, init: RequestInit, serverKey: string) {
  return fetch(`${SUPABASE_URL}/rest/v1/${path}`, { ...init, headers: authHeaders(serverKey, init.headers), cache: "no-store" });
}
...
export async function GET() {
  ...
  const response = await db(
    "beauty_code_characters?select=beauty_code,nickname,image_url,image_url_en,image_url_ja,type_description&order=beauty_code.asc",
    { method: "GET" },
    serverKey,
  );
  ...
  return NextResponse.json({
    ok: true,
    characters: CODES.map((code) => map.get(code) ?? { beauty_code: code, nickname: "", image_url: null, image_url_en: null, image_url_ja: null, type_description: "" }),
  });
}

async function uploadImage(file: FormDataEntryValue | null, beautyCode: string, suffix: "" | "-en" | "-ja", serverKey: string) {
  ...
  const objectPath = `${beautyCode.toLowerCase()}${suffix}.${ext}`;
  const response = await fetch(`${SUPABASE_URL}/storage/v1/object/beauty-code-characters/${objectPath}`, {
    method: "POST",
    headers: authHeaders(serverKey, { "Content-Type": file.type, "x-upsert": "true", "cache-control": "max-age=60" }),
    body: await file.arrayBuffer(),
    cache: "no-store",
  });
  if (!response.ok) throw new Error(await response.text());
  return `${SUPABASE_URL}/storage/v1/object/public/beauty-code-characters/${objectPath}?v=${Date.now()}`;
}
...
export async function POST(request: Request) {
  ...
  try {
    const [imageUrl, imageUrlEn, imageUrlJa] = await Promise.all([
      uploadImage(form.get("image"), beautyCode, "", serverKey),
      uploadImage(form.get("imageEn"), beautyCode, "-en", serverKey),
      uploadImage(form.get("imageJa"), beautyCode, "-ja", serverKey),
    ]);

    const payload: Record<string, string> = { nickname, type_description: typeDescription, updated_at: new Date().toISOString() };
    if (imageUrl) payload.image_url = imageUrl;
    if (imageUrlEn) payload.image_url_en = imageUrlEn;
    if (imageUrlJa) payload.image_url_ja = imageUrlJa;

    const updateResponse = await db(
      `beauty_code_characters?beauty_code=eq.${encodeURIComponent(beautyCode)}&select=beauty_code,nickname,image_url,image_url_en,image_url_ja,type_description`,

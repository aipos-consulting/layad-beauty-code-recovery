...
type Row = { brand?: string | null; product_name: string; category?: string | null; product_url?: string | null };
...
function cleanUrl(v?:string|null){ const s=(v??"").trim(); if(!s)return null; try{ const u=new URL(s); return u.protocol==="http:"||u.protocol==="https:"?u.toString():null; }catch{return null;} }

async function findProduct(name:string,key:string){
  const res=await db(`products?normalized_name=eq.${encodeURIComponent(norm(name))}&deleted_at=is.null&select=id,canonical_name,product_url,brand,category,verification_status&limit=1`,{method:"GET"},key);
  if(!res.ok) throw new Error(await res.text());
  return (await res.json() as Array<{id:string;canonical_name:string|null;product_url:string|null;brand:string|null;category:string|null;verification_status:string}>)[0]??null;
}
...
export async function POST(request:NextRequest){
  ...
  for(let i=0;i<items.length;i++){
    const row=items[i]; const name=(row.product_name??"").trim(); const normalized=norm(name); const productUrl=cleanUrl(row.product_url);
    if(!name){results.push({index:i,name:"",status:"invalid",message:"상품명을 입력해 주세요."});continue;}
    if((row.product_url??"").trim()&&!productUrl){results.push({index:i,name,status:"invalid",message:"상품 링크는 http 또는 https 주소로 입력해 주세요."});continue;}
    ...
    try{
      ...
      if(existing){
        ...
        const patchBody={brand:row.brand?.trim()||existing.brand||null,category:row.category?.trim()||existing.category||null,product_url:productUrl||existing.product_url||null,updated_at:new Date().toISOString()};
        const patch=await db(`products?id=eq.${existing.id}`,{method:"PATCH",headers:{Prefer:"return=minimal"},body:JSON.stringify(patchBody)},key);
        if(!patch.ok) throw new Error(await patch.text());
        if(count===16){results.push({index:i,name,id:existing.id,status:"completed",message:"이미 생성된 상품",fit_count:count,product_url:patchBody.product_url});continue;}
        results.push({index:i,name,id:existing.id,status:"queued",message:"ChatGPT 분석 대기",fit_count:count,product_url:patchBody.product_url});continue;
      }
      const ins=await db("products",{method:"POST",headers:{Prefer:"return=representation"},body:JSON.stringify({canonical_name:name,normalized_name:normalized,product_url:productUrl,brand:row.brand?.trim()||null,category:row.category?.trim()||null,verification_status:"unverified"})},key);
      if(!ins.ok) throw new Error(await ins.text()); const created=(await ins.json() as Array<{id:string}>)[0];
      results.push({index:i,name,id:created?.id,status:"queued",message:"ChatGPT 분석 대기",fit_count:0,product_url:productUrl});

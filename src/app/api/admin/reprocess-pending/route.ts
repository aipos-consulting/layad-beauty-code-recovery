...
const ANALYSIS_VERSION = "layad-v13-display-calibrated-v1";
const BEAUTY_CODES = ["OGPV","OGPE","OGCV","OGCE","OMPV","OMPE","OMCV","OMCE","DGPV","DGPE","DGCV","DGCE","DMPV","DMPE","DMCV","DMCE"] as const;
const UUID = /^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i;
const ACTIONABLE = new Set(["submitted","collecting_reviews","analyzing","failed","insufficient_reviews"]);
...
type PendingRequest = { id:string; session_id:string; product_id:string|null; input_type:"name"|"url"; input_value:string; status:string };
...
async function analyzeWithFallback(openai:string,inputValue:string){
  ...
  console.warn("Admin reprocess web analysis failed; using low-confidence fallback",primaryError);
  try{return {...await oneOpenAICall(openai,buildPrompt(inputValue,true),false,7000),mode:"fallback" as const};}
  catch(fallbackError){console.error("Admin reprocess fallback failed",fallbackError);throw primaryError instanceof Error?primaryError:fallbackError;}
}
function applyClearCategorySignal(inputValue:string,category:string|undefined,s:TraitScores):TraitScores{
  ...
  if(/프라이머|primer|컬러\s*코렉터|color\s*corrector|컨실러|concealer|픽서|fixer|세팅\s*파우더|setting\s*powder/.test(signal)){
    if(next.P===next.C || (next.P===50&&next.C===50)){next.P=66;next.C=44;}
  }
  if(/올인원|all[- ]?in[- ]?one|one\s*step|톤업\s*선|tone[- ]?up\s*sunscreen|멀티밤|multi\s*balm/.test(signal)){
    if(next.P===next.C || (next.P===50&&next.C===50)){next.P=44;next.C=66;}
  }
  return next;
}
...
function calibrateDisplayScore(raw:number){return Math.max(0,Math.min(100,Math.round(raw*0.8+32)));}
...
export async function POST(request:NextRequest){
  ...
  try{
    ...
    if(!UUID.test(requestId)) return NextResponse.json({ok:false,message:"재처리할 요청 ID가 올바르지 않습니다."},{status:400});

    const rr=await db(`product_analysis_requests?id=eq.${encodeURIComponent(requestId)}&select=id,session_id,product_id,input_type,input_value,status&limit=1`,{method:"GET"},key);
    if(!rr.ok) throw new Error(`대기 요청 조회 실패: ${rr.status}`);
    const row=(await rr.json() as PendingRequest[])[0];
    if(!row?.product_id) return NextResponse.json({ok:false,message:"대기 요청에 연결된 상품이 없습니다."},{status:404});
    originalStatus=row.status;
    if(row.status==="completed") return NextResponse.json({ok:true,status:"completed",cached:true,requestId,rowProductId:row.product_id});
    if(!ACTIONABLE.has(row.status)) return NextResponse.json({ok:false,message:`현재 상태(${row.status})는 재처리할 수 없습니다.`},{status:409});

    const fitBefore=await db(`product_type_fits?product_id=eq.${row.product_id}&select=beauty_code&limit=17`,{method:"GET"},key);
    const existingFits=fitBefore.ok?new Set(((await fitBefore.json()) as Array<{beauty_code:string}>).map(v=>v.beauty_code)).size:0;
    if(existingFits===16){
      await db(`product_analysis_requests?product_id=eq.${row.product_id}&status=in.(submitted,collecting_reviews,analyzing,failed,insufficient_reviews)`,{method:"PATCH",headers:{Prefer:"return=minimal"},body:JSON.stringify({status:"completed",error_message:null,updated_at:new Date().toISOString()})},key);
      return NextResponse.json({ok:true,status:"completed",cached:true,requestId,fitCount:16});
    }
    if(existingFits>0) return NextResponse.json({ok:false,code:"PARTIAL_FITS_PRESENT",message:`부분 적합도 데이터(${existingFits}/16)가 있어 자동 재처리를 중단했습니다.`},{status:409});

    const sr=await db(`test_sessions?id=eq.${row.session_id}&select=beauty_code&limit=1`,{method:"GET"},key);
    if(!sr.ok) throw new Error(`Beauty Code 조회 실패: ${sr.status}`);
    const beautyCode=String(((await sr.json()) as Array<{beauty_code?:string}>)[0]?.beauty_code??"");
    if(!/^[OD][GM][PC][VE]$/.test(beautyCode)) return NextResponse.json({ok:false,message:"기존 요청의 Beauty Code를 확인할 수 없습니다."},{status:409});
    ...
    let inputValue=row.input_value.trim();
    if(row.input_type==="url") inputValue=await resolveUrl(inputValue);
    ...
    const fits=BEAUTY_CODES.map(code=>{const raw=scoreForCode(code,s);return {beauty_code:code,raw_fit_score:raw,fit_score:calibrateDisplayScore(raw)};});
    const axes=[
    ...
    const verify=await db(`product_type_fits?product_id=eq.${row.product_id}&select=beauty_code&limit=17`,{method:"GET"},key);
    if(!verify.ok) throw new Error(`16유형 검증 실패: ${verify.status}`);
    const fitCount=new Set(((await verify.json()) as Array<{beauty_code:string}>).map(v=>v.beauty_code)).size;
    if(fitCount!==16){
      await db(`product_analysis_requests?id=eq.${requestId}`,{method:"PATCH",headers:{Prefer:"return=minimal"},body:JSON.stringify({status:originalStatus,updated_at:new Date().toISOString()})},key);
      throw new Error(`16유형 저장 검증 실패: ${fitCount}/16`);
    }

    await db(`product_analysis_requests?product_id=eq.${row.product_id}&status=in.(submitted,collecting_reviews,analyzing,failed,insufficient_reviews)`,{method:"PATCH",headers:{Prefer:"return=minimal"},body:JSON.stringify({status:"completed",error_message:null,updated_at:new Date().toISOString()})},key);
    return NextResponse.json({ok:true,status:"completed",cached:false,analysisMode:mode,requestId,productId:row.product_id,productName:canonical,fitCount,confidence,reviewCount:evidence,timings:{openAiMs,finalizeMs,totalMs:Date.now()-startedAll}});
  }catch(error){
    ...
    const message=isTimeout?"상품 분석 서버가 혼잡합니다. 잠시 후 다시 시도해 주세요.":isMalformed?"상품 분석 결과를 완성하지 못했습니다. 다시 한 번 시도해 주세요.":error instanceof Error?error.message:"재처리에 실패했습니다.";
    console.error("Admin pending reprocess failed",error);
    return NextResponse.json({ok:false,code:isTimeout?"REPROCESS_TIMEOUT":isMalformed?"REPROCESS_INCOMPLETE":"REPROCESS_FAILED",message,timings:{totalMs:Date.now()-startedAll}},{status:isTimeout?504:500});
  }
}

...
export default function MyPage() {
  ...
  useEffect(() => { setPending(Boolean(localStorage.getItem(PENDING_KEY))); void load(); }, []);
    ...
    <main className="min-h-screen bg-[#fff8f8] px-5 py-10 text-[#382d2d] sm:px-8">
      <div className="mx-auto max-w-4xl">
        <header className="flex flex-wrap items-end justify-between gap-4">
          <div><p className="text-xs font-semibold tracking-[.22em] text-[#b97b88]">MY PAGE</p><h1 className="mt-2 text-3xl font-semibold">나의 LAYAD</h1><p className="mt-2 text-sm text-[#806f72]">{data?.user.email}</p></div>
        ...
        <section className="mt-7 rounded-[2rem] bg-white p-7 shadow-[0_18px_50px_rgba(120,70,80,0.09)]">
          ...
          {current ? <div className="mt-6 rounded-3xl bg-[#fff3f5] p-7 text-center"><p className="text-5xl font-semibold tracking-[.18em] text-[#d88c9c]">{current.beauty_code}</p><p className="mt-3 text-sm text-[#806f72]">{new Date(current.created_at).toLocaleDateString("ko-KR")} 저장</p></div> : <div className="mt-6 rounded-3xl border border-dashed border-[#e6cfd4] p-7 text-center"><p className="text-sm text-[#806f72]">아직 저장된 Beauty Code가 없습니다.</p><Link href="/test" className="mt-4 inline-block rounded-full bg-[#d88c9c] px-5 py-2.5 text-sm font-semibold text-white">테스트 시작</Link></div>}
        </section>
        ...
        <div className="mt-7 grid gap-7 md:grid-cols-2">
          ...
          <section className="rounded-[2rem] bg-white p-7 shadow-[0_18px_50px_rgba(120,70,80,0.09)]"><h2 className="text-xl font-semibold">저장한 상품</h2><div className="mt-5 space-y-3">{data?.products.length ? data.products.map(product => <div key={product.id} className="rounded-2xl bg-[#fffafa] px-4 py-3"><p className="font-semibold">{product.product_name || product.product_ref}</p><p className="mt-1 text-xs text-[#8a7a7d]">{product.beauty_code ? `${product.beauty_code} 기준` : ""}{typeof product.fit_score === "number" ? ` · 적합도 ${product.fit_score}` : ""}</p></div>) : <p className="text-sm text-[#806f72]">아직 저장한 상품이 없습니다.</p>}</div></section>
        </div>
      </div>
    </main>
  );

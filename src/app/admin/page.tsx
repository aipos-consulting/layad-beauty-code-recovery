  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        if (active) setData({ ok: false, code: "NETWORK_ERROR" });
      } finally {
          ...
          <div className="space-y-7 p-5 sm:p-8">
            <section>
              ...
              <p className="mt-2 text-sm text-[#7b6d70]">
                {loading ? "Supabase 데이터를 불러오는 중입니다." : data?.ok ? `실제 저장 데이터 기준 · 총 ${totalUsers}개 익명 세션` : "Supabase 데이터를 불러오지 못했습니다."}
              </p>

              {!loading && !data?.ok ? (
                <div className="mt-4 rounded-2xl border border-rose-200 bg-rose-50 p-4 text-sm text-rose-700">
                  {data?.code === "SUPABASE_NOT_CONFIGURED" ? "Vercel의 Supabase 환경변수를 확인하고 다시 배포해 주세요." : "관리자 통계 조회 중 오류가 발생했습니다."}
                </div>
              ) : null}
                ...
                <div className="mt-5 grid grid-cols-2 gap-3 sm:grid-cols-4">
                  {typeStats.map((item, index) => (
                    <article key={item.code} className="rounded-2xl border border-[#eadfe1] bg-[#fffafa] p-4">
                      <div className="flex justify-between"><b className="text-sm">{item.code}</b><span className="text-[11px] text-[#8b7b7e]">{index + 1}위</span></div>
            ...
            <section className="grid gap-6 xl:grid-cols-[1.15fr_1fr]">
                ...
                <div className="mt-5 space-y-3">
                  {topProducts.length ? topProducts.map((product) => (
                    <div key={product.name} className="flex items-center gap-3 rounded-2xl border border-[#eee5e7] p-4">
                      ...
                      <span className={`rounded-full border px-3 py-1 text-[11px] font-semibold ${statusClass[product.status] ?? statusClass.submitted}`}>{statusLabel[product.status] ?? product.status}</span>
                    </div>
              ...
              <article className="rounded-3xl border border-[#eadfe1] bg-white p-5 shadow-sm sm:p-6">
                <h3 className="text-lg font-semibold">운영 우선순위</h3>

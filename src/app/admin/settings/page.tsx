  ...
  return <main className="min-h-screen bg-[#f7f4f4] p-5 text-[#382d2d] sm:p-8">
    <div className="mx-auto max-w-6xl">
      ...
      <section className="mt-5 rounded-3xl border border-[#eadfe1] bg-white p-6 shadow-sm">
        <div className="flex flex-wrap items-end justify-between gap-4"><div><h2 className="text-lg font-semibold">이번 달 일별 AI 비용 추세</h2><p className="mt-1 text-xs text-[#7b6d70]">{usage?.month ?? "현재 월"} · 오너가 비용 증가 패턴을 직접 확인할 수 있습니다.</p></div><div className="text-right text-xs text-[#7b6d70]">완료 {usage?.localRuns?.completed ?? 0} · 실패 {usage?.localRuns?.failed ?? 0} · 총 실행 {usage?.localRuns?.total ?? 0}</div></div>
        {loading ? <p className="mt-6 text-sm text-[#7b6d70]">OpenAI 비용을 확인하고 있습니다.</p> : !usage?.ok ? <div className="mt-6 rounded-2xl bg-amber-50 p-4 text-sm leading-6 text-amber-800"><b>Cost Control 검증 필요</b><br />{usage?.message ?? usage?.code ?? "비용 데이터를 확인할 수 없습니다."}<br /><span className="text-xs">이 상태에서는 상품 자동분석 API도 안전 차단하도록 설계되어 있습니다.</span></div> : (usage.daily?.length ?? 0) === 0 ? <p className="mt-6 text-sm text-[#7b6d70]">이번 달 OpenAI 비용 기록이 아직 없습니다.</p> : <div className="mt-6 space-y-3">{usage.daily?.map(row => <div key={row.date} className="grid grid-cols-[82px_1fr_82px] items-center gap-3 text-xs"><span className="text-[#7b6d70]">{row.date.slice(5)}</span><div className="h-3 overflow-hidden rounded-full bg-[#f3e7e9]"><div className="h-full min-w-[2px] rounded-full bg-[#d88c9c]" style={{ width: `${Math.max(1, row.costUsd / maxDaily * 100)}%` }} /></div><span className="text-right font-semibold">{money(row.costUsd)}</span></div>)}</div>}
      </section>

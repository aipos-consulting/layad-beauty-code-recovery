...
type Row = { brand:string; product_name:string; category:string };
type Result = { index:number; name:string; id?:string; status:string; message:string; fit_count?:number };
type ApiResponse = { ok:boolean; total?:number; queued?:number; completed?:number; failed?:number; results?:Result[]; message?:string };

const emptyRow=():Row=>({brand:"",product_name:"",category:""});
    ...
    <div className="mx-auto max-w-7xl space-y-6">
      <header className="flex flex-col gap-4 xl:flex-row xl:items-end xl:justify-between">
        <div>
          ...
          <p className="mt-2 max-w-3xl text-sm leading-6 text-[#74676a]">브랜드와 상품명만 등록하세요. 1개부터 최대 100개까지 등록할 수 있으며, 실행하면 신규 상품은 ChatGPT 분석 대기 상태로 등록됩니다. 이미 분석이 완료된 상품은 다시 생성하지 않습니다.</p>
        </div>
      ...
      <section className="grid gap-4 md:grid-cols-4">
        {[{k:"현재 입력",v:`${entered} / 100`},{k:"실행 조건",v:"1개 이상이면 실행"},{k:"중복 상품",v:"이미 생성 표시 · 제외"},{k:"분석 방식",v:"ChatGPT 사전분석"}].map(x=><div key={x.k} className="rounded-2xl border border-[#eadfe1] bg-white p-5"><p className="text-xs font-semibold text-[#9b8d90]">{x.k}</p><p className="mt-2 text-lg font-semibold text-[#46383b]">{x.v}</p></div>)}
      </section>

      <section className="rounded-2xl border border-[#eadfe1] bg-white p-5 md:p-6">
        <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
          <div><h2 className="text-lg font-semibold">상품 등록 목록</h2><p className="mt-1 text-sm text-[#817477]">상품명은 필수입니다. 브랜드와 카테고리는 알고 있는 경우 입력하세요.</p></div>
        ...
        <div className="mt-5 overflow-x-auto">
          <table className="w-full min-w-[820px] text-left text-sm">
            <thead><tr className="border-b border-[#eadfe1] text-xs text-[#988b8e]"><th className="w-14 px-3 py-3">No.</th><th className="px-3 py-3">브랜드</th><th className="px-3 py-3">상품명 *</th><th className="px-3 py-3">카테고리</th><th className="w-20 px-3 py-3"></th></tr></thead>
            <tbody>{rows.map((row,i)=><tr key={i} className="border-b border-[#f1e8ea]"><td className="px-3 py-3 text-[#988b8e]">{i+1}</td><td className="px-3 py-2"><input value={row.brand} onChange={e=>update(i,"brand",e.target.value)} placeholder="예: 정샘물" className="w-full rounded-xl border border-[#e2d7d9] px-3 py-2.5 outline-none focus:border-[#b86b7d]"/></td><td className="px-3 py-2"><input value={row.product_name} onChange={e=>update(i,"product_name",e.target.value)} placeholder="예: 에센셜 스킨 누더 쿠션" className="w-full rounded-xl border border-[#e2d7d9] px-3 py-2.5 outline-none focus:border-[#b86b7d]"/></td><td className="px-3 py-2"><input value={row.category} onChange={e=>update(i,"category",e.target.value)} placeholder="예: cushion" className="w-full rounded-xl border border-[#e2d7d9] px-3 py-2.5 outline-none focus:border-[#b86b7d]"/></td><td className="px-3 py-2"><button onClick={()=>removeRow(i)} className="text-xs font-semibold text-[#9a6672]">삭제</button></td></tr>)}</tbody>
          </table>
        </div>

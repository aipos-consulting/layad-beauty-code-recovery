        ...
        <section className="rounded-2xl border border-[#eadfe1] bg-white p-4 shadow-sm">
          <form onSubmit={submitSearch} className="grid gap-3 md:grid-cols-[minmax(240px,1fr)_180px_180px_auto]">
            <input
              ...
              placeholder="이메일 검색"
              className="rounded-xl border border-[#e3d8da] bg-white px-4 py-3 text-sm outline-none focus:border-[#c88494]"
            />
            ...
            <select value={locale} onChange={e => { setPage(1); setLocale(e.target.value); }} className="rounded-xl border border-[#e3d8da] bg-white px-3 py-3 text-sm">
              <option value="all">Locale 전체</option>
              {(data?.locales ?? []).map(x => <option key={x} value={x}>{x}</option>)}
            </select>
            ...
            <table className="w-full min-w-[1100px] border-collapse text-left text-sm">
              <thead className="bg-[#faf4f5] text-[#65585b]">
                <tr>
                  ...
                  <th className="px-4 py-3 font-semibold">Beauty Code</th>
                  <th className="px-4 py-3 text-right font-semibold">저장상품</th>
                  <th className="px-4 py-3 font-semibold">가입일</th>
                  <th className="px-4 py-3 font-semibold">최근 활동</th>
                </tr>
              </thead>
              <tbody>
                {loading && !data ? (
                  ...
                  <tr key={m.id} className="border-t border-[#f0e7e9] hover:bg-[#fffafb]">
                    ...
                    <td className="px-4 py-3 text-right tabular-nums">{m.savedProducts.toLocaleString()}</td>

...
const copy: Record<Locale, Record<string, string>> = {
  ko: {
    ...
    delayed: "공개 리뷰를 확인하고 분석하는 데 예상보다 시간이 더 걸리고 있습니다. 요청은 정상적으로 저장되어 있으므로 잠시 후 결과를 다시 확인해 주세요.",
    ...
    delayedTitle: "실제 리뷰 분석이 계속 진행되고 있습니다",
    analyzingTitle: "실제 공개 리뷰를 수집·분석하고 있습니다",
    collectingTitle: "상품과 기존 분석 결과를 확인하고 있습니다",
    savingTitle: "상품 신청을 접수하고 있습니다",
    analyzingDesc: "신규 상품은 공개된 실제 사용자 리뷰를 찾아 사용감과 상품 특성을 분석한 뒤, 회원님의 Beauty Code와 비교합니다. 그래서 몇 초의 분석 시간이 필요합니다.",
    collectingDesc: "이미 분석된 상품인지 먼저 확인합니다. 기존 분석이 있으면 즉시 결과를 보여드리고, 신규 상품만 실제 공개 리뷰 분석을 시작합니다.",
    savingDesc: "신청 정보를 저장한 뒤 기존 분석 결과가 있는지 바로 확인합니다.",
    reasonTitle: "왜 시간이 필요한가요?",
    reasonDesc: "LAYAD는 임의 점수를 만들지 않습니다. 공개된 실제 사용자 리뷰를 확인하고, 리뷰 근거를 Beauty Code 4개 축으로 분석한 뒤 16개 유형 적합도를 계산합니다.",
    selectedProduct: "선택하신 상품", steps: "상품 확인|공개 리뷰 수집|리뷰 특성 분석|16유형 적합도 계산",
  ...
  en: {
    ...
    delayed: "Review collection and analysis is taking longer than expected. Your request is saved, so please check the result again shortly.",
    ...
    delayedTitle: "Real review analysis is still in progress",
    analyzingTitle: "Collecting and analyzing real public reviews",
    collectingTitle: "Checking the product and existing analysis",
    savingTitle: "Your product request is being submitted",
    analyzingDesc: "For a new product, LAYAD finds public consumer reviews, analyzes usage experience and product characteristics, and compares the evidence with your Beauty Code. This takes a few seconds.",
    collectingDesc: "We first check whether this product has already been analyzed. Existing results are shown immediately; only new products require review analysis.",
    savingDesc: "We save your request and immediately check for an existing analysis.",
    reasonTitle: "Why does this take a few seconds?",
    reasonDesc: "LAYAD does not generate an arbitrary score. It checks real public consumer reviews, maps the evidence to the four Beauty Code axes, and calculates fit across all 16 types.",
    selectedProduct: "Selected product", steps: "Verify product|Collect public reviews|Analyze review features|Calculate 16-type fit",
  ...
  ja: {
    ...
    delayed: "公開レビューの確認と分析に予想より時間がかかっています。リクエストは保存されていますので、しばらくしてから結果を再確認してください。",
    ...
    delayedTitle: "実際のレビュー分析を続けています",
    analyzingTitle: "公開されている実際のレビューを収集・分析しています",
    collectingTitle: "商品と既存の分析結果を確認しています",
    savingTitle: "商品申請を受け付けています",
    analyzingDesc: "新規商品は公開されている実際のユーザーレビューを確認し、使用感と商品特性を分析してからBeauty Codeと比較します。そのため数秒の分析時間が必要です。",
    collectingDesc: "すでに分析済みの商品か先に確認します。既存結果があればすぐ表示し、新規商品のみレビュー分析を行います。",
    savingDesc: "申請情報を保存し、既存の分析結果があるかすぐ確認します。",
    reasonTitle: "なぜ数秒かかるのですか？",
    reasonDesc: "LAYADは任意の点数を作りません。公開された実際のユーザーレビューを確認し、4つのBeauty Code軸で根拠を分析した後、16タイプの適合度を計算します。",
    selectedProduct: "選択した商品", steps: "商品確認|公開レビュー収集|レビュー特性分析|16タイプ適合度計算",
      ...
      <section className="mx-auto w-full max-w-3xl rounded-3xl bg-white p-6 text-[#382d2d] shadow-2xl sm:p-8">
        ...
        {!done && !stopped ? (
          <div className="mt-6 rounded-2xl border border-[#f0d6dc] bg-[#fff8f9] px-4 py-4 text-sm leading-6 text-[#6f5c60]">
            <p className="font-semibold text-[#a94f65]">{t.reasonTitle}</p>
            <p className="mt-1">{t.reasonDesc}</p>
          </div>
        ) : null}

        {!done && !stopped ? <div className="mt-5 space-y-3 text-sm">{t.steps.split("|").map((label, index) => { const number = index + 1; const active = number <= stageNumber; return <div key={label} className="flex items-center gap-3 rounded-2xl bg-[#fffafa] px-4 py-3"><span className={`flex h-6 w-6 items-center justify-center rounded-full text-xs font-semibold ${active ? "bg-[#d88c9c] text-white" : "bg-[#eadfe1] text-[#9b898c]"}`}>{number}</span><span className={active ? "font-medium" : "text-[#9b898c]"}>{label}</span></div>; })}</div> : null}

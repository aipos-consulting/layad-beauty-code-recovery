...
const labels = {
  ko: { title: "친구에게 테스트 공유하기", copy: "링크 복사", kakao: "카카오톡", naver: "네이버카페", instagram: "공식 인스타 보러가기", copied: "결과 링크가 복사되었습니다." },
  en: { title: "Share the test with friends", copy: "Copy link", kakao: "KakaoTalk", naver: "Naver Cafe", instagram: "Visit official Instagram", copied: "Result link copied." },
  ja: { title: "友だちにテストをシェア", copy: "リンクをコピー", kakao: "KakaoTalk", naver: "Naver Cafe", instagram: "公式Instagramを見る", copied: "結果リンクをコピーしました。" },
} as const;
  ...
  return createPortal(
    <section className="mx-auto mt-9 w-full max-w-xl border-t border-[#f1dfe2] bg-[#fff3f5] px-4 pt-8 text-center">
      ...
      <div className="mt-4 flex items-start justify-center gap-6 sm:gap-8">
        {!isAndroid ? <a href={shareUrl(code)} className="flex min-w-[62px] flex-col items-center gap-1.5 bg-[#fff3f5] text-xs font-medium text-[#6f6164] no-underline" aria-label={text.kakao}><KakaoIcon /><span>{text.kakao}</span></a> : null}

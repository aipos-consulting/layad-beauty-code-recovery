...
function resultUrl(code: string) { return `${window.location.origin}/result/${code}`; }

async function makeResultImage(code: string) {
  const canvas = document.createElement("canvas"); canvas.width = 1080; canvas.height = 1080;
  const ctx = canvas.getContext("2d"); if (!ctx) throw new Error("canvas unavailable");
  ctx.fillStyle = "#fff8f8"; ctx.fillRect(0, 0, 1080, 1080); ctx.textAlign = "center";
  ...
  ctx.fillStyle = "#9b858a"; ctx.font = "500 30px sans-serif"; ctx.fillText("What is your Beauty Code?", 540, 760); ctx.font = "500 28px sans-serif"; ctx.fillText("layad16.com", 540, 840);
...
export default function ResultShareBridge() {
  const { locale } = useLanguage(); const text = labels[locale];
  const [mount, setMount] = useState<HTMLElement | null>(null); const [code, setCode] = useState(""); const [status, setStatus] = useState("");
  useEffect(() => {
    const locate = () => {
      ...
      const nextCode = heading.textContent?.trim() ?? ""; const resultSection = heading.closest("section");
    ...
    locate(); const observer = new MutationObserver(locate); observer.observe(document.body, { childList: true, subtree: true, characterData: true }); return () => observer.disconnect();
  }, []);
  async function copyLink() { await navigator.clipboard.writeText(resultUrl(code)); setStatus(text.copied); }
  function lineShare() { const url = encodeURIComponent(resultUrl(code)); const message = encodeURIComponent(`LAYAD BEAUTY CODE ${code}`); window.open(`https://social-plugins.line.me/lineit/share?url=${url}&text=${message}`, "_blank", "noopener,noreferrer"); }
  function kakaoShare() {
    const key = process.env.NEXT_PUBLIC_KAKAO_JAVASCRIPT_KEY; if (!key) { setStatus(text.kakaoMissing); return; }
    const send = () => { if (!window.Kakao) return; if (!window.Kakao.isInitialized()) window.Kakao.init(key); window.Kakao.Share.sendDefault({ objectType: "feed", content: { title: `LAYAD BEAUTY CODE ${code}`, description: "나의 Beauty Code 결과를 확인해 보세요.", imageUrl: `${window.location.origin}/layad-logo.svg`, link: { mobileWebUrl: resultUrl(code), webUrl: resultUrl(code) } }, buttons: [{ title: "결과 보기", link: { mobileWebUrl: resultUrl(code), webUrl: resultUrl(code) } }] }); };
    if (window.Kakao) { send(); return; } const script = document.createElement("script"); script.src = "https://t1.kakaocdn.net/kakao_js_sdk/2.8.3/kakao.min.js"; script.crossOrigin = "anonymous"; script.onload = send; document.head.appendChild(script);
  }
  async function shareImage() {
    try { const blob = await makeResultImage(code); const file = new File([blob], `LAYAD-${code}.png`, { type: "image/png" }); if (navigator.canShare?.({ files: [file] }) && navigator.share) { await navigator.share({ title: `LAYAD BEAUTY CODE ${code}`, text: `My Beauty Code is ${code}`, url: resultUrl(code), files: [file] }); return; } if (navigator.share) { setStatus(text.imageFallback); await navigator.share({ title: `LAYAD BEAUTY CODE ${code}`, url: resultUrl(code) }); return; } const href = URL.createObjectURL(blob); const a = document.createElement("a"); a.href = href; a.download = `LAYAD-${code}.png`; a.click(); URL.revokeObjectURL(href); } catch { setStatus(text.imageFallback); }
  }

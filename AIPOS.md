...
## 11. LAYAD 제품 적합도 AI 분석 기준 — Source of Truth (2026-08-24)

### 11.1 기준선 확정

2026-08-24 오너가 제공한 **「LAYAD Beauty Code 제품 적합도 AI 분석 기준」**을 LAYAD 제품 적합도 분석의 공식 Source of Truth로 확정한다.

이 기준 확정 이후에는 과거 실험 과정에서 사용한 v5~v8의 임시 프롬프트, 임의 가중치, 특정 제품 보정값, 단순 보수관계 계산을 공식 기준으로 사용하지 않는다.

분석의 기본 순서는 다음과 같다.

`제품의 객관적 특성 → 실제 사용자 경험 → Beauty Code 축별 적합성 → 가중치 적용 → 16유형 적합도`

단순 키워드 빈도나 하나의 통합 신호값으로 네 축을 동일하게 계산하지 않는다.

### 11.2 공식 가중치

- O / D — Skin Type: **30%**
- G / M — Finish Preference: **25%**
- P / C — Makeup Style: **25%**
- V / E — Makeup Variability: **20%**
- 총합: **100%**
...
실제 사용자 리뷰를 가장 중요하게 사용한다. 특히 리뷰어가 자신의 피부 타입을 직접 밝힌 경우 신뢰도를 높인다.

- O: 유분·번들거림 억제, 피지 무너짐 감소, 산뜻함, T존 유지 등
- D: 촉촉함, 당김·들뜸·각질 부각 감소, 수분 유지, 건조 부위 밀착 등
- 매트 피니시 자체를 D 부적합으로 처리하지 않는다.

#### G / M — 상세페이지 50% + 리뷰 50%

상세페이지에서 브랜드가 의도한 피니시를 먼저 파악하고 실제 사용자 리뷰로 검증한다.

- G: Glow, Radiant, Dewy, Luminous, 윤광, 물광, 광채 등
- M: Matte, Semi-matte, Soft matte, Blur, 보송, 벨벳, 번들거림 조절 등
- 브랜드 주장과 리뷰가 충돌하면 반복성과 구체성이 높은 실제 사용 경험으로 보정한다.

#### P / C — 카테고리·상품명 + 상세페이지 + 리뷰

제품이 메이크업 루틴에서 수행하는 역할을 우선 판단한다.

- P: 프라이머, 컬러 코렉터, 컨실러, 픽서, 메이크업 베이스, 세팅 파우더 등 정교한 완성도를 위한 추가 단계에 상대적으로 유리
- C: 톤업 선크림, 선케어+베이스, 쿠션 파운데이션, 멀티밤, 올인원, 원스텝 등 단계 축소에 상대적으로 유리
- P는 “사용하기 어려움”을 뜻하지 않는다.
- C는 “성능이 단순함”을 뜻하지 않는다.
- P/C는 제품 우열이 아니라 메이크업 과정에서의 역할과 효율 차이다.

#### V / E — E 기본값 + V 가감 방식

V/E는 다른 세 축보다 제품 선택을 크게 좌우하지 않으며, E를 기본값으로 두고 변화 대응 가능성과 전체 리뷰 경향을 통해 V를 판단한다.

- V 가점: 사용량 조절, 레이어링, 부분 사용, 계절·피부 컨디션별 대응, 타 제품과 조합, 다양한 루틴 활용, 변화에 예민한 사용자의 긍정 경험
- V 감점: 환경에 따라 결과가 크게 흔들림, 특정 조건에서만 적합, 사용량 허용범위가 좁음, 밀림·뭉침, 제품 궁합을 심하게 탐, 예민한 사용자의 반복 부정 경험
- **V 가점은 E 감점으로 전환하지 않는다.**
- **V 감점은 E 가점으로 전환하지 않는다.**
- V와 E를 단순히 `100 - 상대점수` 방식으로 계산하지 않는다.

### 11.4 AI 실행 구조

OpenAI API는 다음 역할을 담당한다.

1. 웹 검색을 통한 상품 식별
2. 공식 상세페이지와 공개 사용자 리뷰 수집/해석
3. O, D, G, M, P, C, V, E의 8개 적합성 점수를 각각 독립적으로 평가
4. 불확실한 항목은 중립값에 가깝게 평가하고 confidence를 낮춤

서버는 AI가 반환한 8개 점수에서 신청자의 Beauty Code에 해당하는 4개 점수를 선택하고 공식 가중치를 적용한다.

예:

`DGPV = D×0.30 + G×0.25 + P×0.25 + V×0.20`

16개 Beauty Code도 같은 방식으로 일관되게 산출한다.
...
- 웹 검색 기반 1차 분석
- 실패 시 제한적 재시도
- 웹검색 실패 시 저신뢰도 fallback 분석
- fallback 결과는 confidence 상한을 낮춰 정상 분석과 구분
- 동일 분석 결과는 저장 후 재사용
- 분석 버전을 명시하여 기준 변경 시 과거 캐시와 구분

### 11.6 분석 버전 및 변경통제

현재 구현 기준 버전:

`layad-v9-owner-prompt-source-of-truth`

GitHub 적용 커밋:

`495fabc5608013381e826c6cafee6cc83c3eb2ff`

앞으로 제품 적합도 프롬프트 또는 가중치를 수정할 때는 다음 절차를 따른다.
...
5. 대표 기준상품 회귀 테스트
6. DB 캐시/분석 버전 영향 확인
7. Production 배포 및 실제 도메인 확인
8. 오너 최종 검증

**금지사항:** 특정 상품 점수를 원하는 값에 맞추기 위한 임의 보정, 공식 문서에 없는 가중치 변경, 한 축의 점수를 다른 축의 자동 보수값으로 전환하는 방식은 오너 승인 없이 적용하지 않는다.
...
| v1.1 | 2026-08-24 | 오너 제공 「LAYAD Beauty Code 제품 적합도 AI 분석 기준」을 제품 적합도 분석의 공식 Source of Truth로 확정. O/D 30%, G/M 25%, P/C 25%, V/E 20% 및 축별 독립 데이터 해석, V/E 비보수관계, v9 분석 메커니즘과 변경통제 절차 반영 |
